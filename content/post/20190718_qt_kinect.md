...
また、普段はC#やTypeScriptといった色々な機能をマネージしてくれる処理系を使っているので、C++を使った実装はなかなか新鮮でした。スマートポインタの存在は知っていたのでメモリ管理はだいぶ楽させてもらえそうですが、そもそもポインタを使うのか実体を使うのかなど、これまであまり考える必要がなかった設計上のポイントがるので刺激的です。
